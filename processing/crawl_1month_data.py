...
def main():
    ...
    # Sau khi df_merged.sort_values(by="Date", inplace=True)
    # Thêm dòng ngày tiếp theo (copy dòng cuối)
    last_row = df_merged.iloc[-1].copy()
    last_row["Date"] = last_row["Date"] + pd.Timedelta(days=1)
    df_merged = pd.concat([df_merged, pd.DataFrame([last_row])], ignore_index=True)
